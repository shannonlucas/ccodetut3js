...
What remains is to actually invoke your `setUpGraphics` function to create the renderer, camera, and scene. Note that the `renderer`, `camera`, and `scene` objects created by the code below are created outside a function which makes them available to all other functions in the same JavaScript file.
